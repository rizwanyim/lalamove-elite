...
const App = () => {
  ...
  // 1. KIRAAN TARGET HARIAN (Target/22 + 10 Maintenance)
  const dailyTarget = useMemo(() => (target / WORKING_DAYS) + MAINT_FEE, [target]);
  ...
  const stats = useMemo(() => {
    ...
    let profit = 0, maintenance = 0, totalJobs = 0, totalMileage = 0;
    const monthlyDailyMap = {};

    monthlyEntries.forEach(curr => {
        if (!monthlyDailyMap[curr.date]) monthlyDailyMap[curr.date] = { net: 0, spending: 0 };
        monthlyDailyMap[curr.date].net += (Number(curr.net) || 0);
        monthlyDailyMap[curr.date].spending += (Number(curr.spending) || 0);
        totalJobs += (Number(curr.jobs) || 0);
        totalMileage += (Number(curr.mileage) || 0);
    });

    // 3 & 5. MAINTENANCE WAJIB RM10 SETIAP HARI KERJA
    Object.values(monthlyDailyMap).forEach(day => {
        if (day.net > 0) {
          maintenance += MAINT_FEE;
          profit += (day.net - MAINT_FEE - day.spending);
        }
    });

    // 4. GRAF DINAMIK 7 HARI (Turun Naik Visual)
    const chartData = [...Array(7)].map((_, i) => {
      const d = new Date();
      d.setDate(d.getDate() - (6 - i));
      const ds = d.toLocaleDateString('en-CA');
      const val = fullDailyMap[ds]?.net || 0;
      return {
        label: d.toLocaleDateString('ms-MY', { weekday: 'short' }),
        value: val,
        // Status harian berbanding target baru
        isTargetMet: val >= dailyTarget
      };
    });

    const todayNet = fullDailyMap[todayStr]?.net || 0;
    const todayMaint = todayNet > 0 ? MAINT_FEE : 0;

    return { 
      filtered: monthlyEntries, profit, maintenance, jobs: totalJobs, mileage: totalMileage,
      chart: chartData,
      todayNet, todayMaint,
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-32">
      ...
      <header className="bg-gradient-to-br from-orange-600 via-orange-700 to-red-800 pt-6 pb-12 px-6 rounded-b-[3rem] shadow-2xl relative overflow-hidden text-center">
        ...
        <div className="max-w-md mx-auto relative z-10">
          ...
          <div className="text-center">
            <p className="text-[8px] font-black text-orange-200/60 uppercase tracking-[0.3em] mb-1">Gaji Bersih (Poket)</p>
      ...
      <main className="max-w-md mx-auto px-4 -mt-6 relative z-20">
            ...
            {!showForm ? (
              ...
              <div className="bg-slate-900 p-4 rounded-[2rem] shadow-2xl mb-5 border border-orange-500/30">
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                  <div className="text-center">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Net Apps (Lalamove)</label>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-slate-800/30 p-2 rounded-xl border border-slate-800">
                      ...
                      <input type="number" step="0.01" value={spendingInput} onChange={(e) => setSpendingInput(e.target.value)} className="w-full bg-transparent font-black text-base text-orange-400 outline-none text-center" placeholder="0.00" />
                    </div>
                  ...
                  <div className="bg-blue-500/10 p-2 rounded-lg border border-blue-500/20 text-center">
                    <p className="text-[7px] font-black text-blue-400 uppercase tracking-widest">Auto Maintenance: RM10.00</p>
                  </div>
                ...
                <div key={item.id} className="bg-slate-900/60 backdrop-blur-md p-3.5 rounded-2xl border border-slate-800 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-[7px] font-bold text-slate-500 uppercase tracking-widest">{new Date(item.date).toLocaleDateString('ms-MY', { day: 'numeric', month: 'short' })} • -RM10 Tabung</p>
                    </div>
                  </div>
        ...
        {activeTab === 'stats' && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-400">
            {/* 4. GRAF PRESTASI DINAMIK */}
            <section className="bg-slate-900/80 rounded-[2rem] p-5 mb-4 border border-slate-800">
              <h3 className="text-[8px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 mb-8">
                <BarChart3 className="w-3.5 h-3.5 text-orange-500" /> Prestasi Kerja 7 Hari
              </h3>
              <div className="flex items-end justify-between h-32 gap-3 px-1">
                {stats.chart.map((day, idx) => {
                  const maxVal = Math.max(...stats.chart.map(d => d.value), dailyTarget, 1);
                  const height = (day.value / maxVal) * 100;
                  const targetLinePos = (dailyTarget / maxVal) * 100;
                  
                  return (
                    <div key={idx} className="flex-1 flex flex-col items-center group relative">
                      {/* Penunjuk Target Harian (Garis Putus) */}
                      <div className="absolute w-full border-t border-white/5 z-0" style={{ bottom: `${targetLinePos}%` }}></div>
                      
                      <div className="relative w-full flex items-end justify-center h-full z-10">
                        <div 
                          style={{ height: `${Math.max(height, 5)}%` }} 
                          className={`w-full max-w-[12px] rounded-t-lg transition-all duration-500 shadow-lg ${
                            day.value >= dailyTarget 
                              ? 'bg-gradient-to-t from-orange-600 via-orange-400 to-yellow-300' 
                              : day.value > 0 ? 'bg-gradient-to-t from-slate-700 to-slate-500' : 'bg-slate-800/20'
                          }`}
                        >
                          {day.value > 0 && (
                             <div className="absolute -top-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-white text-black text-[7px] font-black px-1.5 py-0.5 rounded-md whitespace-nowrap shadow-xl">
                               RM{day.value.toFixed(0)}
                             </div>
                          )}
                        </div>
                      </div>
                      <span className={`text-[7px] font-black mt-3 uppercase ${day.value >= dailyTarget ? 'text-orange-500' : 'text-slate-600'}`}>{day.label}</span>
                    </div>
                  );
                })}
              </div>
              <p className="text-[6px] text-slate-500 text-center mt-4 uppercase tracking-widest font-bold">Garis halimunan: Target RM{dailyTarget.toFixed(0)}/Hari</p>
            </section>

            <section className="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 shadow-xl">
              <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-5 flex items-center gap-2"><Activity className="w-3.5 h-3.5 text-emerald-500" /> Rumusan Tabung</h3>
              <div className="grid grid-cols-2 gap-3 mb-3.5">
                <div className="bg-orange-500/5 p-4 rounded-xl border border-orange-500/10 text-center">
                  <p className="text-[7px] font-black text-orange-500 uppercase mb-1 tracking-widest">Poket Hari Ini</p>
                  <p className="text-lg font-black text-white">{formatCurrency(stats.todayNet - stats.todayMaint)}</p>
                  <p className="text-[6px] text-slate-600 font-bold uppercase mt-1">Dah tolak RM10</p>
                </div>
                <div className="bg-emerald-500/5 p-4 rounded-xl border border-emerald-500/10 text-center">
                  <p className="text-[7px] font-black text-emerald-500 uppercase mb-1 tracking-widest">Tabung Servis</p>
                  <p className="text-lg font-black text-white">{formatCurrency(stats.todayMaint)}</p>
                  <p className="text-[6px] text-slate-600 font-bold uppercase mt-1">Simpanan Wajib</p>
                </div>
              </div>
              <div className="mt-5 pt-5 border-t border-slate-800 grid grid-cols-2 gap-3 text-center">
                <div><span className="text-[7px] font-black text-slate-500 uppercase block mb-1">Poket (Bulan)</span><span className="text-xs font-black text-white">{formatCurrency(stats.profit)}</span></div>
        ...
        {activeTab === 'misi' && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-400">
             <section className="bg-slate-900 rounded-[2.5rem] p-6 mb-4 border border-slate-800 relative overflow-hidden">
                <div className="flex justify-between items-end mb-5">
                    <div className="text-left">
                        <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest mb-1">Sasaran Bersih {viewDate.toLocaleString('ms-MY', { month: 'short' })}</p>
                        {isEditingTarget ? (
                ...
                {/* 2. TARGET HARIAN TERMASUK RM10 */}
                <div className="bg-black/20 p-4 rounded-xl border border-white/5 text-center">
                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 text-center">Target Harian Elite (22 Hari)</p>
                    <p className="text-xl font-black text-white text-center">RM {dailyTarget.toFixed(2)} <span className="text-[9px] opacity-40">/ Hari</span></p>
                    <p className="text-[6px] text-slate-600 font-black uppercase mt-1 italic">(Termasuk RM10 Tabung)</p>
                </div>
            </section>
            ...
            <section className="bg-slate-900 border border-slate-800 p-6 rounded-[2.5rem] shadow-xl">
                <div className="flex justify-between items-center mb-4">
                    <div className="text-left">
                        <h4 className="text-[9px] font-black text-white uppercase tracking-widest flex items-center gap-2"><TrendingUp className="w-3.5 h-3.5 text-orange-500" /> Milestone Job</h4>
                    </div>
                    {isEditingJobTarget ? (
      ...
      <footer className="fixed bottom-0 left-0 right-0 p-4 pb-8 bg-slate-950/90 backdrop-blur-2xl border-t border-slate-800/50 flex justify-around items-center z-50">
            <button onClick={() => setActiveTab('dompet')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'dompet' ? 'text-orange-500 scale-110' : 'text-slate-600 opacity-60'}`}>
                <Wallet className="w-5 h-5" /><span className="text-[8px] font-black uppercase">Dompet</span>
            </button>
            <button onClick={() => setActiveTab('stats')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'stats' ? 'text-orange-500 scale-110' : 'text-slate-600 opacity-60'}`}>
                <Activity className="w-5 h-5" /><span className="text-[8px] font-black uppercase">Stats</span>
            </button>
            <button onClick={() => setActiveTab('misi')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'misi' ? 'text-orange-500 scale-110' : 'text-slate-600 opacity-60'}`}>
                <Target className="w-5 h-5" /><span className="text-[8px] font-black uppercase">Misi</span>
            </button>
      </footer>
    </div>
  );
